...
public final class View extends Observable implements IView, KeyListener {
	...
	final Rectangle gameView = new Rectangle(0, 0, 21, 11);
	 ...
	 */
	/**
	 * Follow the player and adjust the view.
	 */
	@Override
	public final void followMyPlayer() {
		// Centre la vue sur le joueur
		int viewWidth = this.gameView.width;
		int viewHeight = this.gameView.height;

		int playerX = this.getmyPlayer().getX();
		int playerY = this.getmyPlayer().getY();

		int newViewX = Math.max(0, Math.min(playerX - viewWidth / 2, this.getmap().getWidth() - viewWidth));
		int newViewY = Math.max(0, Math.min(playerY - viewHeight / 2, this.getmap().getHeight() - viewHeight));

		this.getgameView().x = newViewX;
		this.getgameView().y = newViewY;
	}
